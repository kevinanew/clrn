...
依次浏览绑定手机（有效国际区号，具体国家由 IP 决定）、邮箱用途说明、改密空表单，再阅读注销影响并点暂不注销。 每次返回后核对原入口或内容，可继续下一分支。

## 定位契约

account-security-item-0..3、phone-number-input、email-address-input、old-password-input、new-password-input、explanation-text、cancel-button；单目标操作前断言可见唯一，业务内容使用文本/表单值断言。

## 关联问题

2026-09-26 已通过真实浏览器探索导航。账号需无绑定手机/邮箱、已有密码；不发码、不改密、不继续注销。
