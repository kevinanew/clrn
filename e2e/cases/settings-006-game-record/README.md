...
从我的进入战绩，等待加载结束，核对无战绩文案并返回。 每次返回后核对原入口或内容，可继续下一分支。

## 定位契约

game-record、no-records-text、progress-hud-activity-indicator、navigation-bar-back-image；单目标操作前断言可见唯一，业务内容使用文本/表单值断言。

## 关联问题

2026-09-26 已通过真实浏览器探索导航。要求账号无战绩；真实空态为「还没有战绩，您可以下拉重新获取」；后台 club 网络弹窗可能阻止返回，测试会明确失败。
