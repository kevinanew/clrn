...
2026-09-26 已使用真实 Playwright 探索线上入口。初始国家区号异步加载并随访问地区变化（本机 +86、GitHub CI +1），等待有效数字区号就绪后再操作。线上切换国家区号会清空已有号码，本案例先选区号再填写，不断言保留旧输入。区号列表有常用地区及字母分组，853 可能重复；使用当前可见且唯一的 852。不注册、不发验证码、不改密、不提交登录凭据。
